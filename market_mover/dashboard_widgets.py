...
def render_single_gauge_html(state: dict) -> str:
    ...
    if isinstance(pct, (int, float)) and pd.notna(pct):
        ...
        rank_text = f"같은 시장의 과거 사건 중 상위 {top_pct}%"
        if pct >= 0.9:
    ...
    needle_len = _R - 13
    pct_text = f"{pct * 100:.0f}%" if state.get("percentile") is not None else "-"
    ...
    text_preview = html.escape(str(state.get("text_preview") or "(원문 없음)")[:220])
    ...
    curated_note = (
        ...
        "뉴스에서 선별한 주요 사건이므로 일반적인 사건보다 큰 사례가 포함될 수 있습니다.</div>"
    ...
    <div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:18px;padding:18px;
                ...
                min-height:440px;box-shadow:0 9px 28px rgba(15,23,42,.055);">
      ...
      <div style="margin-top:14px;background:linear-gradient(145deg,#172033,#0f172a);border-radius:14px;
        ...
        <div style="position:relative;width:{_CX*2}px;height:{_CY+43}px;margin:-3px auto 0;">
          <svg viewBox="0 0 {_CX*2} {_CY+43}" width="{_CX*2}" height="{_CY+43}">
            ...
            <text x="{_CX}" y="{_CY+28}" text-anchor="middle" font-size="19" font-weight="800" fill="#ffffff">{pct_text}</text>
          </svg>
        </div>
      ...
      <div style="font-size:11px;color:#758195;margin-top:12px;">게시물 주제 · {topic}</div>
      <div style="font-size:12px;color:#475569;line-height:1.55;margin-top:5px;height:56px;overflow:hidden;">{text_preview}</div>
      {curated_note}
    </div>
    <style>
